...
bool SampleRenderV2::FileHandler::ReadTextFile(std::string_view path, std::string* content)
{
	bool loaded = false;
...
bool SampleRenderV2::FileHandler::WriteTextFile(std::string_view path, std::string content)
{
	bool stored = false;
...
bool SampleRenderV2::FileHandler::ReadBinFile(std::string_view path, std::byte** content, size_t* fileSize)
{
	bool loaded = false;
	std::ifstream fileStream;
	fileStream.exceptions(std::ifstream::badbit | std::ifstream::failbit);
...
bool SampleRenderV2::FileHandler::WriteBinFile(std::string_view path, std::byte* content, size_t dataSize)
{
	bool stored = false;
	std::ofstream fileStream;
	fileStream.exceptions(std::ofstream::badbit);
...
bool SampleRenderV2::FileHandler::FileExists(std::string_view path)
{
	bool loaded = false;
	std::ifstream fileStream;
